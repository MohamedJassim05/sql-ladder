...
39) Repeat the previous problem with the following stipulations:
    ...
    * Give each column a human-readable column name (for example, "Average Age")
		...
		select count(age) as "Count Age", round(avg(age),2) as "Average Age" , min(age) as "Min Age", max(age)as "Max Age" from pets
...
select avg(salary) from employees where cast(salary AS REAL) and startdate > 2010
...
select printf('$%,.0f',salary) || 'USD' as Mean_Salary_USD,
printf('$%,.0f',salary*0.97) || 'fr' as Mean_salary_CHF 
from employees
...
select species, count(age) as "TOTALNUM", avg(age) as "AVERAGE_AGE", min(age) as "MIN_AGE", max(age) as "MAX_AGE"
from pets
group by species
...
select job, avg(salary)/1.65 as average_salary
from employees
group by job
...
select job , count(*) , avg(salary), min(salary) , max(salary) 
from employees
group by job 
...
select customer , sum(unit_price*quantity) as totalrevenue
from transactions
group by customer
